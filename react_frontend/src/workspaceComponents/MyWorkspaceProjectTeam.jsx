// src/workspaceComponents/MyWorkspaceProjectTeam.jsx
import React from 'react';
import { FaCrown, FaUsers, FaUserMinus, FaUserPlus } from 'react-icons/fa';
...
const MyWorkspaceProjectTeam = ({
  ...
  return (
    <div className="divide-y divide-gray-100 dark:divide-gray-800/30 p-4">
      {/* Managers */}
      <div className="py-3">
        <div className="flex justify-between items-center mb-2">
          ...
          {canManage && (
            <button onClick={onAddManager} className="text-xs text-[#0d9488] hover:text-[#14b8a6] transition font-medium">Add</button>
          )}
        </div>
        {projectManagers.map(m => (
          <div key={m._id} className="flex items-center gap-3 py-2 group">
            ...
            {canManage && projectManagers.length > 1 && (
              <button onClick={() => onRemoveManager(m._id)} className="p-1 text-red-500 dark:text-red-400 opacity-0 group-hover:opacity-100 transition"><FaUserMinus className="text-sm" /></button>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="py-3">
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1">
            <FaUsers className="text-[#0d9488]" /> Team ({teamMembers.length})
          </span>
          {canManage && (
            <button onClick={onAddMember} className="text-xs text-[#0d9488] hover:text-[#14b8a6] transition font-medium">Add</button>
          )}
        </div>
        {teamMembers.map(m => {
          const user = m.user || m;
          ...
          return (
            <div key={memberId} className="flex items-center gap-3 py-2 group">
              ...
              {canManage && (
                <button onClick={() => onRemoveMember(memberId)} className="p-1 text-red-500 dark:text-red-400 opacity-0 group-hover:opacity-100 transition"><FaUserMinus className="text-sm" /></button>
              )}
            </div>
          );
        })}
