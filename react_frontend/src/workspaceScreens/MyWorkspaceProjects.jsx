...
import {
  ...
  FaMagic, FaExclamationTriangle, FaCheck,
} from 'react-icons/fa';
  ...
  const handlePlan = async () => {
    ...
    try {
      ...
      if (res.plan.warnings?.length > 0) {
        toast.error(`${res.plan.warnings.length} warning(s) — some assignments were dropped.`);
      }
    } catch (err) {
          ...
          <>
            ...
            {isPlanning && (
              <p className="text-[10px] text-gray-500 dark:text-gray-500 text-center mt-2">
                AI is reading your workspace members and drafting the plan…
              </p>
            )}
          </>
        ) : (
