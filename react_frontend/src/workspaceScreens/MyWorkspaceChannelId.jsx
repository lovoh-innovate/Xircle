...
const QuotedReplyBlock = ({ replyData, isOwn, brandColor, onJump }) => {
  ...
  return (
    <button
      ...
      className={`block w-full text-left mb-1.5 px-2.5 py-1.5 rounded-lg border-l-2 text-xs cursor-pointer transition ${isOwn ? "bg-black/10 border-white/60 hover:bg-black/20" : "bg-black/[0.04] dark:bg-white/[0.06] hover:bg-black/[0.07] dark:hover:bg-white/[0.1]"}`}
...
const MediaMessage = ({
  ...
  const renderDesktopMenu = () => (
    ...
    >
      ...
      <button
        onClick={() => { setShowMenu(false); onCopy(message); }}
        className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/30 rounded-lg transition w-full"
      >
      ...
      {isStarred ? (
        <button
          onClick={() => { setShowMenu(false); onUnstar(message._id); }}
        ...
        <button
          onClick={() => { setShowMenu(false); onStar(message._id); }}
          className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/30 rounded-lg transition w-full"
        >
      ...
      {isArchived ? (
        <button
          onClick={() => { setShowMenu(false); onUnarchive(message._id); }}
        ...
        <button
          onClick={() => { setShowMenu(false); onArchive(message._id); }}
          className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/30 rounded-lg transition w-full"
        >
...
// ─── Chat Details Bottom Sheet (Group) ──────────────────────────────
...
const MyWorkspaceChannelId = () => {
  ...
  const mergeMessagesIntoState = useCallback(
    ...
    [userInfo?._id, chat?.participants]
  );
  ...
  const renderEditBar = () => {
    ...
    return (
      <div className="flex items-center justify-between px-3 py-2 mb-2 bg-teal-50 dark:bg-teal-900/20 rounded-lg border border-teal-200 dark:border-teal-700/40">
        <div className="flex-1 min-w-0">
          <span className="text-xs font-semibold text-teal-600 dark:text-teal-400">Editing message</span>
          <p className="text-sm text-gray-700 dark:text-gray-300 truncate">{editContent}</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleCancelEdit} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-white transition">
            <FaTimes />
          </button>
          <button onClick={handleSaveEdit} className="text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300 transition">
            <FaSave />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="h-dvh bg-gray-50 dark:bg-[#0b0b10] flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-[#0f0f12] h-full overflow-hidden">
        {/* ─── Header ─── */}
        <header
          className="fixed lg:sticky top-0 left-0 right-0 lg:left-auto lg:right-auto z-20 flex items-center justify-between px-4 py-3 border-b border-gray-200/60 dark:border-gray-800/60 bg-white/80 dark:bg-[#0f0f12]/80 backdrop-blur-xl text-gray-800 dark:text-white flex-shrink-0 cursor-pointer relative"
        ...
        >
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="font-semibold text-base text-gray-800 dark:text-gray-100 truncate">{chat?.name || "Unnamed Channel"}</h2>
          ...
          <div className="flex gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-white transition"
            >
              <FaPhone />
            </button>
            <button
        ...
        <div className="relative flex-1 overflow-hidden">
          <div
            ...
            className="h-full overflow-y-auto px-4 py-4 space-y-3 pt-20 lg:pt-3 pb-24 lg:pb-3 transition-all duration-200 ease-in-out"
            style={{
          ...
          <form onSubmit={handleSendMessage} className="flex items-end gap-2 py-2">
                          ...
                          {savedStickersData.stickers.map((sticker) => (
                            ...
                            >
                              {sticker.type === "image" ? (
                                ...
                                <video src={sticker.fileUrl} className="w-full h-full object-contain" muted />
                              )}
                            </button>
                          ))}
                        </div>
            ...
            {message.trim() || editingMessageId || slash.pending.length > 0 ? (
              ...
              <button
                ...
                onPointerCancel={(e) => {
                  if (isRecordingRef.current && !recordingPaused) quickSendRecording();
                  micActionLockRef.current = false;
                }}
