...
const MyWorkspaceChatId = () => {
  ...
  const isRecentDuplicateMedia = useCallback(
    (signature) => {
      ...
      return localMessages.some((m) => {
        const isOwnMsg = m.sender?._id === userInfo?._id || m.sender === userInfo?._id;
  ...
  useEffect(() => {
    ...
    const handleMessageRead = ({ chatId: readChatId, messageId, readBy }) => {
      ...
      setLocalMessages((prev) => prev.map((msg) => (msg._id === messageId ? { ...msg, _read: true } : msg)));
    };
      ...
      setLocalMessages((prev) =>
        prev.map((msg) => {
          if (msg._id === messageId) {
            return {
              ...msg,
              reactions: (msg.reactions || []).filter((r) => !(r.user === userId && r.emoji === emoji)),
            };
          }
          return msg;
        })
  ...
  // ─── Polling ──────────────────────────────────────────────────────
  // Only poll on latest page; older pages fetched via pagination.
  useEffect(() => {
    if (!chatId) return;
  ...
  return (
    <div className="h-dvh bg-gray-50 dark:bg-[#0b0b10] flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-[#0f0f12] h-full overflow-hidden">
        {/* Header */}
        <header className="fixed lg:sticky top-0 left-0 right-0 lg:left-auto lg:right-auto z-20 flex items-center justify-between px-4 py-3 border-b border-gray-200/60 dark:border-gray-800/60 bg-white/80 dark:bg-[#0f0f12]/80 backdrop-blur-xl text-gray-800 dark:text-white flex-shrink-0 relative">
