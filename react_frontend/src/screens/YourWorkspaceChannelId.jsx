    ...
    >
      {loading ? (
        <div className="p-3 animate-pulse">
          <div
            className={`h-3 w-2/3 rounded-full mb-2 ${isOwn ? "bg-white/20" : "bg-gray-300 dark:bg-gray-700"}`}
          />
          <div
            className={`h-2 w-1/3 rounded-full ${isOwn ? "bg-white/20" : "bg-gray-300 dark:bg-gray-700"}`}
          />
        </div>
      ) : data ? (
        <>
          ...
          <div className="p-2.5">
            {data.title && (
              <p
                className={`text-xs font-semibold line-clamp-1 ${isOwn ? "text-white" : "text-gray-800 dark:text-gray-200"}`}
              >
                {data.title}
              </p>
            )}
            {data.description && (
              <p
                className={`text-[11px] line-clamp-2 mt-0.5 ${isOwn ? "text-white/70" : "text-gray-500 dark:text-gray-400"}`}
              >
                {data.description}
              </p>
            )}
            <p
              className={`text-[10px] mt-1 flex items-center gap-1 ${isOwn ? "text-white/50" : "text-gray-400 dark:text-gray-500"}`}
            >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-[#0b0b10]/80 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-[#14141a] border border-gray-200 dark:border-gray-800/60 rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-200/60 dark:border-gray-800/60">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800/60 rounded-lg text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:hover:text-white transition"
          >
            ...
            availableMembers.map((member) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-xl transition ${isSelected ? "bg-teal-50 dark:bg-[#0d9488]/20" : "hover:bg-gray-50 dark:hover:bg-gray-800/30"}`}
                >
                  <div className="relative flex-shrink-0">
                    {user?.profile ? (
                      <img
                  ...
                  {isSelected && (
                    <FaCheck className="text-sm text-teal-600 dark:text-[#0d9488]" />
                  )}
                </button>
              );
            })
        ...
        <div className="flex gap-3 p-4 border-t border-gray-200/60 dark:border-gray-800/60">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 border border-gray-300 dark:border-gray-700/60 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800/30 transition"
          >
            Cancel
          </button>
          <button
...
const AudioPlayer = ({
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 w-full py-0.5">
      ...
      <span
        className={`text-[8px] sm:text-[10px] flex-shrink-0 ${isOwn ? "text-white/70" : "text-gray-500 dark:text-gray-400"}`}
      >
    ...
    if (!sticker) {
      return (
        <div className={`flex items-start gap-3 ${isOwn ? "flex-row-reverse" : ""}`}>
          ...
          >
            <div
              className={`px-4 py-2.5 rounded-2xl text-sm break-words w-full ${isOwn ? "text-white" : "bg-gray-100 dark:bg-gray-800/60 text-gray-800 dark:text-gray-200"}`}
          ...
          >
            ...
            <div className="relative">
              ...
              {!isMobile && isHovering && (
                <>
                  <div
                    className={`absolute top-1/2 -translate-y-1/2 z-[60] ${isOwn ? "right-full mr-2" : "left-full ml-2"}`}
                  >
                    <div className="relative">
                      <button
                        onClick={(e) => {
...
const ChatDetailsSheet = ({
  ...
  const isGroupAdmin =
    chat.participants?.some(
      (p) =>
        (p.user?._id === userInfo?._id || p.user === userInfo?._id) &&
        p.role === "admin",
    ) || canManageWorkspace;
            ...
            <ul className="space-y-2">
              {participants.map((p) => {
                ...
                const canPromote =
                  ...
                  !canManageWorkspace;
                const canDemote =
                  canManage && !isCurrentUser && isAdmin && !canManageWorkspace;
                    ...
                    <div className="flex-1">
                      ...
                      {isAdmin && (
                        <span className="ml-2 text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 px-2 py-0.5 rounded-full border border-yellow-200 dark:border-yellow-700/40">
                          <FaCrown className="inline text-[10px] mr-0.5" />{" "}
                          Admin
                        </span>
                      )}
                      {isCurrentUser && (
...
const YourWorkspaceChannelId = () => {
  ...
  const isWorkspaceOwner =
    workspaceData?.workspace?.owner?._id === userInfo?._id;
  const isWorkspaceAdmin = workspaceData?.workspace?.members?.some(
    (m) =>
      (m.user?._id || m.user) === userInfo?._id &&
      m.role === "Admin" &&
      m.status === "active",
  );
...
const messagesMentioningMe = useMemo(() => {
    ...
    if (messagesData?.messages) {
      ...
      setLocalMessages((prev) => {
        ...
        const merged = newMessages.map((msg) => {
          const isOwn =
            msg.sender?._id === userInfo?._id || msg.sender === userInfo?._id;
          const obj = {
            ...msg,
            _sent: true,
            _pending: false,
          ...
          if (isOwn) {
            ...
            if (
              otherIds.some((id) =>
                msg.readBy?.some((r) => r.user === id || r.user?._id === id),
              )
            ) {
              obj._read = true;
            }
          ...
          setLocalMessages((prev) =>
            prev.map((m) => {
              if (m._id === msgId) {
                return {
                  ...m,
                  starredBy: [...(m.starredBy || []), userInfo._id],
                };
              }
              return m;
            }),
      ...
      setLocalMessages((prev) =>
        prev.map((m) => {
          if (m._id === msgId) {
            return {
              ...m,
              archivedBy: [...(m.archivedBy || []), userInfo._id],
            };
          }
          return m;
        }),
          ...
          setLocalMessages((prev) =>
            prev.map((m) => {
              if (m._id === msgId) {
                return {
                  ...m,
                  archivedBy: (m.archivedBy || []).filter(
                    (id) => id !== userInfo._id,
                  ),
                };
              }
  ...
  const handleUnarchiveMessage = useCallback(
    (msg) => {
      const msgId = msg._id;
      setLocalMessages((prev) =>
        prev.map((m) => {
  ...
  const handleDeleteGroup = (chatId) => {
    setConfirmModal({
      ...
      title: "Delete Group",
      message:
        "Are you sure you want to permanently delete this group and all its messages? This cannot be undone.",
      onConfirm: async () => {
        try {
  ...
  const renderMessagesWithDividers = () => {
    ...
    sorted.forEach((msg) => {
      ...
      const sender = msg.sender?._id
        ? resolveSender(msg.sender._id)
        : resolveSender(msg.sender);
      const isOwn =
        msg.sender?._id === userInfo?._id || msg.sender === userInfo?._id;
      elements.push(
        <MediaMessage
  ...
  return (
    <div className="h-dvh bg-gray-50 dark:bg-[#0b0b10] flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-[#0f0f12] h-full overflow-hidden">
        ...
        <div className="relative flex-1 overflow-hidden">
          <div
            ...
            className="h-full overflow-y-auto px-4 py-4 space-y-3 pt-20 lg:pt-3 pb-30 lg:pb-3 transition-all duration-200 ease-in-out"
            style={{
      ...
      <PromptModal
        ...
        onConfirm={(value) =>
          promptModal.onConfirm && promptModal.onConfirm(value)
        }
      ...
      <MessageActionModal
        ...
        isStarred={
          actionModal.message?.starredBy?.some((id) => id === userInfo?._id) ||
          false
        }
        isArchived={
          actionModal.message?.archivedBy?.some((id) => id === userInfo?._id) ||
          false
        }
        onDelete={handleDeleteMessage}
        onArchive={handleArchiveMessage}
