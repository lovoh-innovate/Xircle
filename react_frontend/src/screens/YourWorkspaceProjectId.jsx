...
const TaskListItem = ({ task, isActive, onClick, brandColor }) => {
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 text-white font-semibold text-lg" style={{ backgroundColor: brandColor }}>
...
const ChatBubble = ({ message, isOwn, brandColor }) => {
  ...
  return (
    <div className={`flex gap-2 mb-4 ${isOwn ? 'flex-row-reverse' : ''}`}>
      <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ backgroundColor: isOwn ? brandColor : '#CBD5E0' }}>
      ...
      <div className={`max-w-[85%] md:max-w-[75%] ${isOwn ? 'items-end' : 'items-start'}`}>
        <div className={`px-4 py-2.5 rounded-2xl shadow-sm break-words overflow-hidden ${isOwn ? 'text-white rounded-br-md' : 'bg-white border border-gray-200 rounded-bl-md'}`} style={isOwn ? { backgroundColor: brandColor } : {}}>
          ...
          {message.links?.length > 0 && (
            <div className="mt-2 flex flex-col gap-1">
              {message.links.map((l, i) => (
                <a key={i} href={l} target="_blank" rel="noopener noreferrer" className="text-xs underline break-all" style={{ color: isOwn ? 'white' : brandColor }}>{l}</a>
              ))}
            </div>
          )}
...
// ─── Task Detail View (WhatsApp‑like) ───────────────────────────────────
const TaskDetailView = ({ task, brandColor, feedbackData, isLoading, userInfo, onBack, onEdit, onDelete, onUpdateProgress, onApproveCompletion, canManage }) => {
  ...
  return (
    <div className="flex flex-col h-full overflow-x-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b bg-white sticky top-0 z-10">
        <button onClick={onBack} className="p-1.5 hover:bg-gray-100 rounded-lg md:hidden"><FaArrowLeft className="text-gray-500 text-sm" /></button>
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-semibold text-base flex-shrink-0" style={{ backgroundColor: brandColor }}>{task.title.charAt(0).toUpperCase()}</div>
        <div className="flex-1 min-w-0">
          <h2 className="text-sm font-bold text-gray-900 truncate">{task.title}</h2>
          <div className="flex items-center gap-1.5 text-xs text-gray-500 mt-0.5">
            <TaskStatusBadge status={task.status} />
            <TaskPriorityBadge priority={task.priority} />
            {task.assignee && <span className="flex items-center gap-1 truncate"><FaUserCheck className="text-[10px]" /> {task.assignee.name}</span>}
          </div>
        </div>
        <div className="relative flex-shrink-0">
          <button onClick={() => setShowMenu(!showMenu)} className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg"><FaEllipsisV className="text-sm" /></button>
          {showMenu && (
            <div className="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border border-gray-200 min-w-[140px] z-20 py-1">
              {canManage && (
                <button onClick={() => { setShowMenu(false); onApproveCompletion(task); }} className="flex items-center gap-2 px-4 py-2 text-sm text-green-600 hover:bg-green-50 w-full"><FaCheckCircle className="text-xs" /> Approve</button>
              )}
              <button onClick={() => { setShowMenu(false); onEdit(task); }} className="flex items-center gap-2 px-4 py-2 text-sm text-blue-600 hover:bg-blue-50 w-full"><FaEdit className="text-xs" /> Edit</button>
              {canManage && (
                <button onClick={() => { setShowMenu(false); onDelete(task._id); }} className="flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 w-full"><FaTrashAlt className="text-xs" /> Delete</button>
              )}
            </div>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 md:py-6 bg-[#efeae2] md:bg-gray-50">
        {isLoading ? (
          <div className="flex justify-center py-8"><div className="w-6 h-6 border-4 border-t-transparent rounded-full animate-spin" style={{ borderTopColor: brandColor }} /></div>
        ) : feedbackList.length === 0 ? (
          <div className="text-center py-12 text-gray-400"><FaCommentDots className="text-3xl mx-auto mb-3" /><p className="text-sm">No feedback yet</p></div>
        ) : (
          feedbackList.map((item, idx) => <ChatBubble key={idx} message={item} isOwn={item.user?._id === userInfo?._id} brandColor={brandColor} />)
        )}
      </div>

      {/* Slim bottom bar */}
      <div className="border-t border-gray-200/80 p-2 flex-shrink-0 bg-white">
        <button onClick={() => onUpdateProgress(task)} className="w-full py-1.5 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-1.5" style={{ backgroundColor: brandColor }}>
...
// ─── Modals (attachments combined with links) ────────────────────────────
...
const CreateTaskModal = ({ isOpen, onClose, projectId, brandColor, assignableMembers, onSuccess }) => {
  ...
  const [linksText, setLinksText] = useState(''); // one URL per line
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5"><FaLink className="inline mr-1" /> Links & Attachments (one URL per line)</label>
            <textarea value={linksText} onChange={e => setLinksText(e.target.value)} rows={3} className="w-full px-4 py-2 border rounded-lg text-sm" placeholder="Paste URLs to documents, images, or any resources" />
          </div>
...
const YourWorkspaceProjectId = () => {
  ...
  if (wLoad || pLoad || tLoad) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="w-10 h-10 border-4 border-t-transparent rounded-full animate-spin" style={{ borderTopColor: brandColor }} />
    </div>
  );
          ...
          <div className="px-4 py-3 border-b bg-white">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <button onClick={() => navigate(`/workspace/${workspaceId}/projects`)} className="p-2 hover:bg-gray-100 rounded-lg"><FaArrowLeft className="text-gray-500" /></button>
                <div className="flex items-center gap-2">
